...
public class GameLobbyVerticle extends AbstractVerticle {
    ...
    /**
     * Given a request containing a valid username, create a player and add it to the records of currently active players.
     * @param message should contain a username that is not already taken.
     */
    private void newPlayer(Message<JsonObject> message) {
        ...
        for (Player p : activePlayers) {
            if (body.getString("username").equals(p.getName())) message.fail(409, ERROR_CONFLICT_USERNAME_ALREADY_TAKEN);
        }
        ...
        message.reply(new PlayerCreatedDTO(temp.getId(), temp.getName()));
    }
